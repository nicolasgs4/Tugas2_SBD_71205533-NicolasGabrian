...
if (isset($_POST['simpan'])) { 
    ...
    if ($id_invoice && $id_produk && $Qty) {
        if ($op == 'edit') { 
            $sql1       = "update detailinvoice set id_invoice = '$id_invoice',id_produk ='$id_produk',Qty = '$Qty' where id_invoice = '$id_invoice'";
            $kurangstock = "update produk set stock_produk = stock_produk - $Qty WHERE id_produk = $id_produk";
            $q1         = mysqli_query($koneksi, $sql1);
            if ($q1) {
                $sukses = "Data berhasil diupdate";
                $q2 = mysqli_query($koneksi,$kurangstock);
            } else {
